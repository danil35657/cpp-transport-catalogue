...
void TransportCatalogue::AddBus(const std::string& bus_name, const std::vector<std::string>& bus_stops, const bool is_roundtrip) {
    Bus bus{bus_name, {}, is_roundtrip};
    ...
    for (size_t i = 0; i < stops_points.size() - 1; ++i) {
...
std::map<std::string_view, TransportCatalogue::Stop*> TransportCatalogue::GetAllStops() {
    return std::map(stopname_to_stop_.begin(), stopname_to_stop_.end());
}

std::map<std::string_view, TransportCatalogue::Bus*> TransportCatalogue::GetAllBuses() {
